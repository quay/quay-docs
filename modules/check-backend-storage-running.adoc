...
[role="_abstract"]
To check the status of your {productname} backend storage and verify access, you can use provider dashboards and CLIs for AWS, GCS, NooBaa, ODF, Ceph, Azure, or OpenStack Swift. Ensure all {productname} instances have access to all S3 storage backends.
...
* link:https://health.aws.amazon.com/health/status[AWS Service Health Dashboard]
* link:https://status.cloud.google.com/[Google Cloud Status Dashboard]
* link:https://azure.status.microsoft/en-us/status[Azure Status Dashboard]
* link:https://www.ibm.com/docs/ro/cmwo/4.3.0.0?topic=services-checking-status[OpenStack Status]
