...
[role="_abstract"]
To deploy a local {productname} instance over IPv6, you can set `FEATURE_LISTEN_IP_VERSION` in `config.yaml` and start the registry container with explicit IPv6 port mappings.
