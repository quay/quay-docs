...
[role="_abstract"]
To import vulnerability database updates into your disconnected {ocp} cluster for a self-managed deployment, you can use the `clairctl` tool with your Clair configuration file to import the updaters bundle. This populates the Clair database with vulnerability data so Clair can scan images without internet access.
