...
[role="_abstract"]
The webhook reference provides the JSON payload format required to trigger builds in {productname}. You must `POST` a `JSON` payload with `commit`, `ref`, and `default_branch` fields to the webhook URL to automatically start a build.
