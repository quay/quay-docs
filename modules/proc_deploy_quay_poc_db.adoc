...
[role="_abstract"]
{productname} requires a database for storing metadata. To configure the database for your proof of concept {productname} deployment, you can create a local PostgreSQL data directory and start a Postgres container with Podman. You must also install the `pg_trgm` extension required by the registry.
