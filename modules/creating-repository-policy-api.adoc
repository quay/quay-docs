...
. Enter the following `POST /api/v1/repository/<repository>/autoprunepolicy/` command to create a new policy that limits the number of tags allowed in a repository:
+
[source,terminal]
----
...
`tagPatternMatches`:: Specifies that the `false` value prunes tags that do not match the given regex pattern. In this example, all tags except those that match `^test.` are pruned.
