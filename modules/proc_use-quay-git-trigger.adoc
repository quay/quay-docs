...
* It is not possible for Quay.io to automatically detect the proper
...
the trigger.
* Setup a _webhook_ that POSTs to the Quay.io endpoint to trigger a
build.
...
SSH public key that Quay.io generates for a custom git trigger. For
example,
