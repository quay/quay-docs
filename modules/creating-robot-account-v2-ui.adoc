...
[role="_abstract"]
To automate access to your repositories, you can create a robot account by using the {productname} v2 UI. Robot accounts generate credentials that container clients can use to push and pull images without a personal login.

.Procedure 

. On the v2 UI, click *Organizations*. 

. Click the name of the organization that you create the robot account for, for example, `test-org`.
