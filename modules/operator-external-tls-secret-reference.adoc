...
With this approach, the {productname} Operator uses the certificate and private key from the referenced Secret instead of embedding `ssl.cert` and `ssl.key` files in the `configBundleSecret` resource. 
