...
You can specify a set of nodes to host infrastructure components only. You apply specific Kubernetes labels to these nodes and then update the infrastructure components to run on only those nodes. In this section, you will see how to manually label and taint infrastructure nodes and then control the placement of Quay pods on these nodes using tolerations. 
