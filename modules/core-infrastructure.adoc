...
[role="_abstract"]
{productname} infrastructure options range from all-in-one to highly available and geo-distributed, on standalone hosts or on {ocp}. You can deploy standalone with automation or use the {productname} Operator for {ocp} for managed deployment, scaling, and integration. {productname} runs on any physical or virtual infrastructure, both on premise or public cloud.
