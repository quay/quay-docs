...
[role="_abstract"]
To run an LDAP {productname} deployment in debug mode and debug LDAP operations, you can use `podman run` with `DEBUGLOG=true` and `USERS_DEBUG=1`. View debug output with the `podman logs` command.
