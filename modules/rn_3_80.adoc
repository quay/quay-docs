...
* When using the proxy cache feature, attempting to mirror `registry.ci.openshift.org/origin` as an anonymous user (or a user that is not logged in) results in the following error: `Failed login to remote registry. Please verify entered details and try again`. This is a known issue and will be fixed in a future version of {productname}. For more information, see link:https://issues.redhat.com/browse/PROJQUAY-4585[PROJQUAY-4585]. 
