...
....
# docker run -p 443:8443 quay.io/redhat/quay:v{productmin} config my-secret-password
....
...
. **Identify the Redis hostname and add other desired settings**: Other setting you can add to complete the setup are as follows. More settings for high availability {productname} deployment that for the basic deployment:
...
* **Server Configuration**: Hostname or IP address to reach the {productname} service, along with TLS indication (recommended for production installations). Recommended for high availability.
