...
[role="_abstract"]
To upload custom SSL/TLS certificates to {productname-ocp}, you can create a `configBundleSecret` resource that includes your `ssl.cert` and `ssl.key files` and reference it from the `QuayRegistry` custom resource.
