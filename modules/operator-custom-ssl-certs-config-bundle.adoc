...
[role="_abstract"]
When you deploy {productname-ocp}, the Operator can use {ocp} Certificate Authority certificates or custom SSL/TLS certificates in the config bundle. 

Configuring custom certificates can be done before or after initial deployment by updating the `configBundleSecret` and setting the `tls` component to `unmanaged` (`managed: false`). By default, when {productname} is deployed on {ocp}, the `tls` component of the `QuayRegistry` custom resource definition (CRD) is set to `managed` (`managed: true`). As a result, {ocp}'s Certificate Authority is used to create HTTPS endpoints and to rotate SSL/TLS certificates.
