...
[role="_abstract"]
To retrieve {productname} configuration settings from the command line, you can enable `FEATURE_SUPERUSER_CONFIGDUMP` and call the `v1/superuser/config` API endpoint with a superuser OAuth 2 access token. 

As a {productname} superuser, you can return all Flask configuration fields that are set, which you can use to show proof of compliance for various security policies, such as PCI-DSS 4.0.
