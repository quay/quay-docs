...
Users will also need to configure Clair v4 when upgrading from 3.3.z to 3.6.z. For more information configuring Clair v4, see link:https://access.redhat.com/documentation/en-us/red_hat_quay/3.6/html-single/upgrade_red_hat_quay/index#manually_deploying_clair[manually deploying Clair] if configuring from an OpenShift deployment, or link:https://access.redhat.com/documentation/en-us/red_hat_quay/3.6/html-single/upgrade_red_hat_quay/index#setting_up_clair_on_a_non_openshift_red_hat_quay_deployment[setting up Clair on a non-OpenShift Red Hat Quay deployment] for standalone deployments.  
...
When upgrading to 3.6.z from 3.3.z, users will need to configure 
