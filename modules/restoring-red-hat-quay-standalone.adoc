...
[id="restoring-red-hat-quay-standalone"]
= Restoring {productname} on standalone deployments

[role="_abstract"]
To restore a standalone {productname} deployment from backup, you can restore configuration files, recreate the PostgreSQL database, and sync blobs to object storage.
...
. Create a new directory that bind-mounts to `/conf/stack` inside of the {productname} container:
+
[source,terminal]
----
...
. Copy the contents of your temporary backup directory created in the backup procedure to the `new-quay-install` directory created in Step 1:
+
[source,terminal]
----
...
`<example_endpoint_url>`:: Specifies the {productname} registry endpoint. The endpoint must be the same before backup and after restore.
+
.Example output:
[source,terminal]
----
...
. Before restarting your {productname} deployment, update the storage settings in your `config.yaml` file:
+
[source,yaml]
----
DISTRIBUTED_STORAGE_CONFIG:
