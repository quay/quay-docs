...
* `REDIS_FLUSH_INTERVAL_SECONDS` defines the time, in seconds, at which the Redis flush worker clears old data. Shorter intervals keep data fresher and help prevent Redis from bloating, while longer intervals reduce flush frequency.
