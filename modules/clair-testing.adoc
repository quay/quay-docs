...
[role="_abstract"]
To confirm that Clair is scanning images on {productname}, you can push a sample image to your registry and open the vulnerability report in the UI.
...
[NOTE]
====
In some cases, Clair shows duplicate reports on images, for example, `ubi8/nodejs-12` or `ubi8/nodejs-16`. This occurs because vulnerabilities with the same name are for different packages. You can expect this behavior with Clair vulnerability reporting. Duplicate reports are not a bug.
====