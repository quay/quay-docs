...
* Create an immutability policy for a repository by using the `POST /api/v1/repository/{repository}/immutabilitypolicy/` endpoint. For example:
+
[source,text]
----
$ curl -X POST -H "Authorization: Bearer <access_token>" -H "Content-Type: application/json" -d '{"tagPattern": ".*", "tagPatternMatches": true}' http://<quay-server.example.com>/api/v1/repository/<repository_name>/immutabilitypolicy/
----
+
.Example output
+
