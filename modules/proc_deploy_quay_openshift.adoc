...
(the password was set as an argument to the quay config container in: quay-enterprise-config.yaml):
...
+
image:../../images/Figure03.png[Set up a {productname} superuser account to do {productname} configuration]
+
...
. **Deploy configuration**: Select `To to deployment rollout`. When prompted, click
...
image:../../images/repo-mirror-config.png[Enable mirroring and require HTTPS and verified certificates]
