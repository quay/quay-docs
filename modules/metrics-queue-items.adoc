...
[id="metrics-queue-items"]
= Queue items

[role="_abstract"]
Use these queue items metrics to monitor the work queues that {productname} uses for exports, garbage collection, notifications, builds, and storage replication.
...
+
--
`exportactionlogs`:: Queued requests to export action logs. These logs are then processed and put in storage. A link is then sent to the requester by email.
...
`dockerfilebuild`:: Queue for {productname} container image builds.
`imagestoragereplication`:: Queued blob to be replicated across multiple storages.
`chunk_cleanup`:: Queued blob segments that need to be deleted. This is only used by some storage implementations, for example, Swift.
--
