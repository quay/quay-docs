...
[role="_abstract"]
To configure access to the Clair database in your disconnected {ocp} cluster for a self-managed deployment, you can determine the database service, forward the database port, and update your Clair `config.yaml` file to use `localhost`. This lets you import the updaters bundle into the database using the `clairctl` tool.
