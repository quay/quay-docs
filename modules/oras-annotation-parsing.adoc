...
[role="_abstract"]
Some OCI media types do not include labels for metadata such as expiration. You can use ORAS annotations with {productname} to embed that metadata in artifacts.
...
`--annotation "quay.expires-after=2d"`:: Specifies that the expiration time is set for 2 days, indicated by `2d`.
`--annotation "expiration = 2d"`:: Specifies that the expiration label is added.
+
.Example output
[source,terminal]
----
