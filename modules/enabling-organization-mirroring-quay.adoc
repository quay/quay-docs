...
[role="_abstract"]
To enable organization mirroring in {productname}, you can set `FEATURE_ORG_MIRROR` to `true` in your `config.yaml` file and restart the registry.

.Procedure

. To enable organization mirroring, set the following configuration fields in your `config.yaml` file:
+
[source,yaml]
----
# ...
...
`SSRF_ALLOWED_HOSTS`:: Specifies the allowed hosts for Server-Side Request Forgery (SSRF) protection. Use this optional field to allow specific hosts to be accessed by the registry.
