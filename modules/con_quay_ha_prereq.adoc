...
** **A: db01: Load balancer and database**: Runs the HAProxy load balancer and a Postgres database. 
+
[NOTE]
====
HAProxy and Postgres are not themselves high availability. Instead, they are used to indicate how you might set up your own load balancer or production database. 
====
