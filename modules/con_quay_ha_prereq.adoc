...
[id="preparing-for-quay-ha"]
= Preparing for {productname} (high availability)

[role="_abstract"]
To prepare for a high availability {productname} deployment, you need compatible database, object storage, load balancer, and RHEL systems that meet the sizing requirements in this guide.

[IMPORTANT]
====
The {productname} configuration tool is unsupported for standalone high availability deployments. Create and maintain the `config.yaml` file manually, as described in this guide. For a full list of configuration fields, see _Additional resources_.
====
...
is available from Crunchy Data for use with {productname}.
...
** Amazon S3 (see _Additional resources_ for details on configuring an S3 bucket policy for Quay)
** Azure Blob Storage
...
[id="using-podman-quay-ha"]
== Using podman

This document uses podman for creating and deploying containers. If you do not have podman available on your system, you should be able to use the equivalent docker commands. For more information on podman and related technologies, see _Additional resources_.

[NOTE]
====
Production quality deployments of {productname} generally use Podman. Docker has not been tested with {productname} {producty}, and will be deprecated in a future release.
====
...
* link:https://access.redhat.com/downloads/content/479/ver=/rhel---9/9.0/x86_64/product-software[Red Hat Enterprise Linux Downloads]
...
* link:https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/8/html-single/building_running_and_managing_containers/index[Building, running, and managing Linux containers on Red Hat Enterprise Linux 8]
