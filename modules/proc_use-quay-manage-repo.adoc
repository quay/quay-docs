...
As a {product-title} user, you can create your own repositories and
make them accessible to other users on your {product-title} instance.
...
without requiring that the client have a {product-title} user account.
...
. Log into your {product-title} user account.
...
your {product-title} registry. They are similar to OpenShift service accounts.
...
. In the Team Members box, type one of the following:
  * A username from an account on the {product-title} registry
...
user logs in to the {product-title} instance, the user moves from the
