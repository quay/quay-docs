...
[source,terminal]
----
$ curl -X POST -k --header 'Content-Type: application/json' \
-H "Authorization: Bearer 6B4QTRSTSD1HMIG915VPX7BMEZBVB9GPNY2FC2ED" \
https://example-registry-quay-quay-enterprise.apps.docs.quayteam.org/api/v1/organization/ \
--data '{"name": "testorg", "contact_email": "testorg@example.com"}'
----
+
The `contact_email` field is optional. If omitted, notifications default to organization owners.
+
Example output:
+
[source,yaml]
