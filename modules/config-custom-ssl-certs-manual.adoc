...
[role="_abstract"]
To add Certificate Authorities to a standalone {productname} container, you can copy CA files into the `extra_ca_certs` directory in your configuration folder and restart the registry container. {productname} uses these certificates to verify TLS connections to external services.
