:_mod-docs-content-type: REFERENCE
[id="updateorgmirrorconfig"]
= updateOrgMirrorConfig

[role="_abstract"]
Update organization mirror configuration.

[id="updateorgmirrorconfig-put"]
== PUT /api/v1/organization/{orgname}/mirror



**Authorizations: **oauth2_implicit (**org:admin**)



[id="updateorgmirrorconfig-request-body-schema"]
== Request body schema (application/json)

Update organization mirror configuration

[options="header", width=100%, cols=".^3a,.^9a,.^4a"]
|===
|Name|Description|Schema
|**is_enabled**
 
_optional_||boolean
|**external_registry_url**
 
_optional_||string
|**external_namespace**
 
_optional_||string
|**robot_username**
 
_optional_||string
|**visibility**
 
_optional_||string
|**sync_interval**
 
_optional_||integer
|**sync_start_date**
 
_optional_||string
|**external_registry_username**
 
_optional_||
|**external_registry_password**
 
_optional_||
|**external_registry_config**
 
_optional_||object
|**repository_filters**
 
_optional_||array of string

|**skopeo_timeout**
 
_optional_||integer
|===


[id="updateorgmirrorconfig-responses"]
== Responses

[options="header", width=100%, cols=".^2a,.^14a,.^4a"]
|===
|HTTP Code|Description|Schema
|200|Successful invocation|
|400|Bad Request|&lt;&lt;_apierror,ApiError&gt;&gt;
|401|Session required|&lt;&lt;_apierror,ApiError&gt;&gt;
|403|Unauthorized access|&lt;&lt;_apierror,ApiError&gt;&gt;
|404|Not found|&lt;&lt;_apierror,ApiError&gt;&gt;
|===

[id="updateorgmirrorconfig-example-command"]
== Example command
[source,terminal]
----
$ curl -X PUT \
  -H "Authorization: Bearer <bearer_token>" \
  -H "Accept: application/json" \
  -H "Content-Type: application/json" \
  -d '{"is_enabled": true, "sync_interval": 7200}' \
  https://<quay-server.example.com>/api/v1/organization/<orgname>/mirror
----
