...
[role="_abstract"]
Configure logging, debugging, and health check variables for troubleshooting {productname}.
...
====
| **ALLOW_PULLS_WITHOUT_STRICT_LOGGING** | Boolean | If true, pulls will still succeed even if the pull audit log entry cannot be written . This is useful if the database is in a read-only state and it is desired for pulls to continue during that time. **Default:**  False
