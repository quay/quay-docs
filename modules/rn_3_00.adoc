...
=== Changes to support running containers "unprivileged"
Previous versions of images required running in "privileged" mode. To remove this restriction some changes were necessary in container config and ports.

* clair-jwt config has moved from /config to /clair/config
* References to addition files, such as certificates, in clair-jwt's config also need to be updated
* quay http and https ports are now 8080 and 8443 respectively
* If using the proxy port on quay it has been moved to 7443

In addition, the move to a RHEL base image means the certificate install path has changed to /etc/pki/ca-trust/source/anchors. Examples running the images have been updated to reflect this.
