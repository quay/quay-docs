...
[role="_abstract"]
To list usage logs or obtain registry size information in {productname}, you can use the superuser logs and registry size API endpoints.
...
* Use the `GET /api/v1/superuser/registrysize/` endpoint to obtain information about the size of the registry:
+
[source,terminal]
----
$ curl -X GET \
...
* Use the `POST /api/v1/superuser/registrysize/` endpoint to define registry size information:
+
[source,terminal]
----
