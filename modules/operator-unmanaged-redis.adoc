...
[role="_abstract"]
To integrate an existing Redis database with your {productname} registry, you can set the redis component to unmanaged and configure `BUILDLOGS_REDIS` and `USER_EVENTS_REDIS` in the `configBundleSecret`. This lets you use your own Redis infrastructure for build logs and user event processing.
