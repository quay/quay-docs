...
[role="_abstract"]
Manage your own Redis infrastructure by using an external Redis database with {productname}. 

To integrate an existing Redis database with your {productname} registry, you can set the `redis` component to unmanaged and configure `BUILDLOGS_REDIS` and `USER_EVENTS_REDIS` in the `configBundleSecret` resource.
