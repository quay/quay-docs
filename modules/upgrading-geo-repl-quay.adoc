...
[role="_abstract"]
To upgrade a geo-replication deployment of standalone {productname}, stop operations on all instances, back up the deployment, then follow the procedure to upgrade each system. Expect intermittent downtime when upgrading to the next y-stream release.
