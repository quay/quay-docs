...
. After confirming that the update has properly worked on System A, initiate the {productname} upgrade on System B and on System C. The Operator upgrade results in an upgraded {productname} installation, and the pods are restarted.
+
[NOTE]
====
