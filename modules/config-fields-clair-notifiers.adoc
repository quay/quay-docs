...
[role="_abstract"]
You can use the Clair notifier configuration fields to set database connections, polling intervals, and webhook, AMQP, or STOMP delivery options in {productname}.
...
[id="config-fields-clair-notifier-example"]
...
[id="config-fields-clair-amqp-example"]
...
[id="config-fields-clair-stomp-example"]
