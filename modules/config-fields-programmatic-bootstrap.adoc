...
[role="_abstract"]
The following configuration fields enable the control of bootstrap OAuth token provisioning for automated {productname} deployments. 

Programmatic bootstrapping remains disabled until you enable it. Standalone deployments store the token in a local file. {productname-ocp} Operator deployments store the token in a Kubernetes Secret that the Operator creates and mounts.
