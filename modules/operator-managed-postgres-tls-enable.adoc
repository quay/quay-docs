...
[role="_abstract"]
To encrypt database traffic for Operator-managed PostgreSQL used by {productname} and Clair, you can set `overrides.tls.enabled: true` on the `postgres` and `clairpostgres` components in the `QuayRegistry` custom resource.
