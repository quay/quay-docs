...
[role="_abstract"]
To integrate an existing PostgreSQL database with your {productname} registry, you can set the `postgres` component to unmanaged and configure the DB_URI in the `configBundleSecret`. This lets you leverage your current database infrastructure instead of using the Operator-managed database.
