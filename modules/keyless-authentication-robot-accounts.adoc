...
[NOTE]
====
The {productname} v2 UI federation modal accepts *Issuer URL* and *Subject* only. For federation audience validation behavior and current API limits, see xref:configuring-federation-audiences[Configuring federation audiences].
====
...
. After you configure federation, exchange an external OIDC access token for a {productname} robot token as described in xref:exchanging-oauth2-robot-account-token[Exchanging an OAuth2 access token for a {productname} robot account token].

For the API request schema, see link:https://docs.redhat.com/en/documentation/red_hat_quay/{producty}/html-single/red_hat_quay_api_reference/index#createorgrobotfederation[createOrgRobotFederation].
