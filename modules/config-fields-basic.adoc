...
[role="_abstract"]
Customize registry branding, contact information, and footer links that appear in the {productname} user interface.
...
| **REGISTRY_TITLE** | String | If specified, the long-form title for the registry. Displayed in frontend of your {productname} deployment, for example, at the sign in page of your organization. Should not exceed 35 characters. **Default:** `Red Hat Quay`
...
| **.logo** (Required) |  String |  Main logo image URL. The header logo defaults to 205x30 PX. The form logo on the {productname} sign in screen of the web UI defaults to 356.5x39.7 PX. **Example:** `/static/img/quay-horizontal-color.svg`
| **.footer_img** | String |  Logo for UI footer. Defaults to 144x34 PX. **Example:** `/static/img/RedHat.svg`
...
|*.PRIVACY_POLICY_URL* | String | Custom privacy policy for on-prem installations. **Example:** `https://example.hr`
|*.SECURITY_URL* | String | Custom security page for on-prem installations. **Example:** `https://example.hr`
| **.ABOUT_URL** | String | Custom about page for on-prem installations. **Example:** `https://example.hr`
|===
