...
[id="metrics-multipart-uploads"]
= Multipart uploads metrics

[role="_abstract"]
Use these multipart upload metrics to monitor blob uploads to object storage and to help identify failures when {productname} cannot complete an upload.
...
| quay_multipart_uploads_started_total | Number of multipart uploads to {productname} storage that started
| quay_multipart_uploads_completed_total | Number of multipart uploads to {productname} storage that completed
|===
