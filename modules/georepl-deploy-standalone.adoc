...
[id="georepl-deploy-standalone"]
= Run {productname} with storage preferences

[role="_abstract"]
To run a standalone {productname} instance with a regional storage preference, you can set the `QUAY_DISTRIBUTED_STORAGE_PREFERENCE` environment variable when you start the container.

.Procedure

. Copy the `config.yaml` file to all machines running {productname}.
