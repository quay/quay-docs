...
[role="_abstract"]
To expand storage capacity for your {productname-ocp} deployment, you can use the {ocp} console to resize the PostgreSQL and Clair PostgreSQL persistent volume claims (PVCs). 

Resizing PostgreSQL and Clair PostgreSQL PVCs lets you increase storage beyond the default 50 GiB allocation when your registry needs more space.

When deploying {productname-ocp}, three distinct PVCs are deployed:
