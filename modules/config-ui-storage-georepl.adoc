...
[role="_abstract"]
To enable storage replication for a standalone {productname} deployment, you can configure distributed storage engines in `config.yaml` and backfill existing image data.

.Procedure

. Update your `config.yaml` file to include the storage engines to which data is replicated. You must list all storage engines to be used:
+
[source,yaml]
----
# ...
