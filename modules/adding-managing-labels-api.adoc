...
. You can add an additional label to a manifest in a given repository with the `POST /api/v1/repository/{repository}/manifest/{manifestref}/labels` command. For example:
+
[source,terminal]
----
$ curl -X POST \
...
. You can delete a label using the `DELETE /api/v1/repository/{repository}/manifest/{manifestref}/labels/{labelid}` command:
+
[source,terminal]
----
$ curl -X DELETE \
