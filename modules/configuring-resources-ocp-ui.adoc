...
. In the `spec.components` field, you can override the resources of all components by setting values for the `.overrides.resources.limits` and the `overrides.resources.requests` fields. You can also specify a `storageClassName` for `postgres` and `clairpostgres` resources. For example:
+
[source,yaml]
----
spec:
