...
. In the `spec.components` field, you can override the resources of all components by setting values for the `.overrides.resources.limits` and the `overrides.resources.requests` fields. You can also specify a `storageClassName` for `postgres`, `clairpostgres`, and `clair` resources. For `postgres` and `clairpostgres`, these fields must be defined during initial installation of the component. For `clair`, `storageClassName` applies to the ephemeral scratch volume used for image layer extraction. For example:
+
[source,yaml]
----
spec:
