...
[role="_abstract"]
To retrieve account information or star and unstar repositories in {productname}, you can use the user API endpoints with an OAuth access token.
...
. Use the `GET /api/v1/user/` endpoint to get user information for the authenticated user.
+
[source,terminal]
----
...
. Use the `GET /api/v1/users/{username}` endpoint to get user information for the specified user.
+
[source,terminal]
----
...
. Use the `POST /api/v1/user/starred` endpoint to star a repository:
+
[source,terminal]
----
...
. Use the `GET /api/v1/user/starred` endpoint to list all starred repositories:
+
[source,terminal]
----
...
. Use the `DELETE /api/v1/user/starred/{repository}` endpoint to delete a star from a repository:
+
[source,terminal]
----
