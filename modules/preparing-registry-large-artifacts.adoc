...
[role="_abstract"]
To accept large AI or ML artifacts in {productname}, you can increase the `minimum_chunk_size_mb` value in your `config.yaml` file after you consult Red Hat Support.
...
Artificial intelligence (AI) or machine learning (ML) artifacts such as large-language models (LLMs), vector graphics, trained model files, or large datasets often require that {productname} administrators modify their registry to suit the needs of pushing such larger artifacts. By default, {productname} uses a minimum chunk size (or the pieces that a large file is split into during upload) of 5 MB. This means that larger layers, for example, 50 GB, result in 10,000 chunks. This can be confirmed based on the following formula:
