:_mod-docs-content-type: REFERENCE
[id="exportrepologs"]
= exportRepoLogs

[role="_abstract"]
Queues an export of the logs for the specified repository.

[id="exportrepologs-post"]
== POST /api/v1/repository/{repository}/exportlogs



**Authorizations: **oauth2_implicit (**repo:admin**)


[id="exportrepologs-path-parameters"]
== Path parameters

[options="header", width=100%, cols=".^2a,.^3a,.^9a,.^4a"]
|===
|Type|Name|Description|Schema
|path|**repository**
 
_required_|The full path of the repository. e.g. namespace/name|string
|===


[id="exportrepologs-query-parameters"]
== Query parameters

[options="header", width=100%, cols=".^2a,.^3a,.^9a,.^4a"]
|===
|Type|Name|Description|Schema
|query|**endtime**
 
_optional_|Latest time for logs. Format: "%m/%d/%Y" in UTC.|string
|query|**starttime**
 
_optional_|Earliest time for logs. Format: "%m/%d/%Y" in UTC.|string
|===


[id="exportrepologs-request-body-schema"]
== Request body schema (application/json)

Configuration for an export logs operation

[options="header", width=100%, cols=".^3a,.^9a,.^4a"]
|===
|Name|Description|Schema
|**callback_url**
 
_optional_|The callback URL to invoke with a link to the exported logs|string
|**callback_email**
 
_optional_|The e-mail address at which to e-mail a link to the exported logs|string
|===


[id="exportrepologs-responses"]
== Responses

[options="header", width=100%, cols=".^2a,.^14a,.^4a"]
|===
|HTTP Code|Description|Schema
|201|Successful creation|
|400|Bad Request|&lt;&lt;_apierror,ApiError&gt;&gt;
|401|Session required|&lt;&lt;_apierror,ApiError&gt;&gt;
|403|Unauthorized access|&lt;&lt;_apierror,ApiError&gt;&gt;
|404|Not found|&lt;&lt;_apierror,ApiError&gt;&gt;
|===

[id="exportrepologs-example-command"]
== Example command
[source,terminal]
----
$ curl -X POST \
  -H "Authorization: Bearer <bearer_token>" \
  -H "Content-Type: application/json" \
  -H "Accept: application/json" \
  -d '{
        "starttime": "2024-01-01",
        "endtime": "2024-06-18",
        "callback_url": "http://your-callback-url.example.com"
      }' \
  "http://<quay-server.example.com>/api/v1/repository/{repository}/exportlogs"
----
