...
[role="_abstract"]
To verify federated robot account access in {productname}, you can log in with the robot token and pull images that the robot is allowed to access.
...
. Log in to your {productname} registry by using the `fed_test+robot1` robot account and the `QUAY_TOKEN` access token. For example:
+
[source,terminal]
----
