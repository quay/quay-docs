...
[id="IBM-P-and-Z-known-issue"]
=== UI Build Log issue for IBM Power & IBM Z

The UI V2 Build feature is not supported with {odf}. It works fine with AWS S3 or any other external object storage. This is a known issue and will be fixed in a future version of {productname}. (link:https://issues.redhat.com/browse/PROJQUAY-6940[*PROJQUAY-6940*]).
