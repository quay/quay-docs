...
[role="_abstract"]
To scan container images for vulnerabilities in your {ocp} cluster, you can install the {cso} from the {ocp} OperatorHub and view vulnerability information in the dashboard. 

The following procedure sets up the {cso} to monitor pods and expose vulnerability data through the Kubernetes API. 
...
After completing this procedure, you are made aware of what images are vulnerable, what you must do to fix those vulnerabilities, and every namespace that the image was run in. Knowing this, you can perform the following actions:
