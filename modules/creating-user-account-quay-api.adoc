...
[role="_abstract"]
To create a user account in {productname} as a superuser, you can use the {productname} API.
...
. Enter the following command to create a new user by using the `POST /api/v1/superuser/users/` endpoint:
+
[source,terminal]
----
...
. Navigate to your {productname} registry endpoint, for example, `quay-server.example.com` and log in with the username and password generated from the API call. In this scenario, the username is `newuser` and the password is `123456789`. Alternatively, you can log in to the registry with the CLI. For example:
+
[source,terminal]
----
