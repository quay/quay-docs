...
[id="backing-up-red-hat-quay-standalone"]
= Backing up {productname} on standalone deployments

[role="_abstract"]
To back up a standalone {productname} deployment, you can archive configuration files, dump the PostgreSQL database, and sync object storage blobs.
