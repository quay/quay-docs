...
[role="_abstract"]
To manage auto-prune policies for your own user namespace in {productname}, you can use the `/api/v1/user/autoprunepolicy/` API endpoints.
...
. Enter the following `POST` command to create a new policy that limits the number of tags for the current user:
+
[source,terminal]
----
