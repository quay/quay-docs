...
However, global read-only superusers cannot view all content on the new UI. For example, the *Superusers* panel that has been added to the {productname} v2 UI is unavailable to global read-only superusers. Updates will be made to the global read-only superuser feature in future versions.