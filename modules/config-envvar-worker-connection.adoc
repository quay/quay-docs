...
[role="_abstract"]
Use worker connection count environment variables to limit simultaneous HTTP connections per {productname} worker process globally or per component. These limits apply either globally or can be scoped to individual components (registry, web UI, or security scanning). 

By default, each worker process allows up to `50` parallel connections. This setting is distinct from the number of _worker processes_. Connection-related environment variables can be configured differently depending on your deployment type:
