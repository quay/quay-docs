...
|**email** + 
_optional_|Organization contact email. Optional email address for billing and quota notifications. Can be shared across multiple organizations. Used as the primary recipient for quota notifications when `FEATURE_QUOTA_NOTIFICATIONS` is enabled. If not set, quota notifications fall back to organization administrator email addresses.|string
|**invoice_email** + 
