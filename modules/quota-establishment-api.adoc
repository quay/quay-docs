...
. View the storage consumed with `GET` data from the */api/v1/repository* endpoint:
...
. Use the */api/v1/repository* endpoint to view the quota report for the repositories in the organization:
+
.Sample command
[source,terminal]
----
...
. Use the */api/v1/organization/{orgname}* endpoint to view the quota information in the organization details:
+
.Sample command
[source,terminal]
----
...
. `POST` data to the */api/v1/organization/{orgname}/quota/{quota_id}/limit* endpoint to set _reject_ and _warning_ limits:
...
. Use the */api/v1/organization/{orgname}/quota* endpoint to view the _reject_ and _warning_ limits:
