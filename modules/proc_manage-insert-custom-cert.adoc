...
= Adding TLS Certificates to the Red Hat Quay Container

To add custom TLS certificates to Red Hat Quay, create a new
directory named `extra_ca_certs/` beneath the Red Hat Quay config
...
When deployed on Kubernetes, Red Hat Quay mounts in a secret as a volume to store
