...
[role="_abstract"]
To uninstall the Container Security Operator from your {ocp} deployment, you must uninstall the Operator and delete the `imagemanifestvulns.secscan.quay.redhat.com` custom resource definition (CRD). 

Without removing the CRD, image vulnerabilities are still reported on the {ocp} *Overview* page.
