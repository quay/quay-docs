:_mod-docs-content-type: REFERENCE
[id="resetrepositorynotificationfailures"]
= resetRepositoryNotificationFailures

[role="_abstract"]
Resets repository notification to 0 failures.

[id="resetrepositorynotificationfailures-post"]
== POST /api/v1/repository/{repository}/notification/{uuid}



**Authorizations: **oauth2_implicit (**repo:admin**)


[id="resetrepositorynotificationfailures-path-parameters"]
== Path parameters

[options="header", width=100%, cols=".^2a,.^3a,.^9a,.^4a"]
|===
|Type|Name|Description|Schema
|path|**repository** + 
_required_|The full path of the repository. e.g. namespace/name|string
|path|**uuid** + 
_required_|The UUID of the notification|string
|===


[id="resetrepositorynotificationfailures-responses"]
== Responses

[options="header", width=100%, cols=".^2a,.^14a,.^4a"]
|===
|HTTP Code|Description|Schema
|204|Successful creation|
|400|Bad Request|&lt;&lt;_apierror,ApiError&gt;&gt;
|401|Session required|&lt;&lt;_apierror,ApiError&gt;&gt;
|403|Unauthorized access|&lt;&lt;_apierror,ApiError&gt;&gt;
|404|Not found|&lt;&lt;_apierror,ApiError&gt;&gt;
|===

[id="resetrepositorynotificationfailures-example-command"]
== Example command

[source,terminal]
----
$ curl -X POST \
  -H "Authorization: Bearer <bearer_token>" \
  https://<quay-server.example.com>/api/v1/repository/<repository>/notification/<uuid>
----
