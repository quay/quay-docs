...
+
*With contact_email (recommended):*
+
[source,terminal]
----
$ curl -X POST -H "Authorization: Bearer <bearer_token>" -H "Content-Type: application/json" \
  -d '{"name": "engineering", "contact_email": "team@example.com"}' \
  "https://<quay-server.example.com>/api/v1/organization/"
----
+
*Without email (notifications default to owners):*
+
[source,terminal]
----
$ curl -X POST -H "Authorization: Bearer <bearer_token>" -H "Content-Type: application/json" \
...
----
+
*With both email fields (contact_email takes precedence):*
+
[source,terminal]
----
$ curl -X POST -H "Authorization: Bearer <bearer_token>" -H "Content-Type: application/json" \
  -d '{"name": "engineering", "email": "old@example.com", "contact_email": "team@example.com"}' \
  "https://<quay-server.example.com>/api/v1/organization/"
----
+
