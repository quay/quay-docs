...
= Managing an organization application by using the {productname} API

[role="_abstract"]
To create, list, update, or delete organization applications in {productname}, you can call the organization applications API endpoints with an OAuth access token.
...
. Use the `POST /api/v1/organization/{orgname}/applications` endpoint to create a new application for your organization. For example:
+
[source,terminal]
----
...
. After creation, you can return application information by using the `GET /api/v1/app/{client_id}` endpoint:
+
[source,terminal]
----
...
. You can delete organization applications with the `DELETE /api/v1/organization/{orgname}/applications/{client_id}` endpoint. For example:
+
[source,terminal]
----
