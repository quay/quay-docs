...
[role="_abstract"]
The following configuration fields define how user accounts behave in your {productname} deployment. 
...
| **USER_RECOVERY_TOKEN_LIFETIME**  | String | The length of time a token for recovering a user accounts is valid **Pattern**: `^[0-9]+(w\|m\|d\|h\|s)$` 
