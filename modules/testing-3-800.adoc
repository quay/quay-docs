...
=== Enabling LDAD restricted users for {productname} 

The `LPAP_RESTRICTED_USER_FILTER` is now available. This configuration field is a subset of the `LDAP_USER_FILTER` configuration field. When configured, allows {productname} administrators the ability to configure Lightweight Directory Access Protocol (LDAP) users as restricted users when {productname} uses LDAP as its authentication provider.

[NOTE]
====
This configuration field only applies when your `AUTHENTICATION_TYPE` is set to `LDAP` in your `config.yaml` file. If your `AUTHENTICATION_TYPE` is set to `Database`, see xref:enabling-feature-restricted-users[Enabling restricted users]. 
====
...
When set, restricted users cannot create organizations or content in their own namespace. Normal permissions apply for an organization's membership, for example, a restricted user will still have normal permissions in organizations based on the teams that they are members of.

[NOTE]
====
This configuration field only applies when your `AUTHENTICATION_TYPE` is set to `Database` in your `config.yaml` file. If your `AUTHENTICATION_TYPE` is set to `LDAP`, see xref:enabling-ldap-restricted-users[Enabling LDAP restricted users]. 
====

.Prerequisites 

`AUTHENTICATION_TYPE` is set to `Database` in your `config.yaml` file. 
...
When this feature is set, specified users are excluded from the `FEATURE_RESTRICTED_USERS` configuration. Use the following procedure to exclude users from the `FEATURE_RESTRICTED_USERS` setting so that they can have `read` and `write` privileges. 
...
==== Viewing an image for vulnerabilities 

Use the following procedure to view the list of vulnerabilities for a scanned image. 

.Prerequisites 

* You have manually deployed Clair. For more information, see link:https://access.redhat.com/documentation/en-us/red_hat_quay/3/html-single/manage_red_hat_quay/index#clair-standalone[Setting up Clair on a non-OpenShift Red Hat Quay deployment].
...
[source,terminal]
----
$ podman tag docker.io/library/busybox quay-server.example.com/quayadmin/clair:4.2.3:test
----
...
[source,terminal]
----
$ podman push quay-server.example.com/quayadmin/clair:4.2.3:test
----

. In the navigation pane of your {productname} deployment, click *Repositories*. 

. Click the name of your repository, for example, *quayadmin/clair:4.2.3*. 

. Click the name of your tag, for example, *test*. 
