...
[role="_abstract"]
Mirroring and geo-replication solve different distribution needs in {productname}. Mirroring syncs selected repositories between separate registries; geo-replication shares one database and replicates blob storage across regions.

For example, a geo-replicated {productname} registry can use two different blob storage endpoints.
...
| Are individual / different role-based access control configurations allowed in each region | No | Yes
|===
