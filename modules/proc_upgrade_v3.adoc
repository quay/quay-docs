...
to upgrade your cluster to v{productmin} you must bring down your entire cluster and
...
. Bring up one quay container to complete the migration to v{productmin}.
. Once the migration is done, make sure the same `config.yaml` is available
on all nodes and bring up the new quay v{productmin} service on those nodes.
