...
[role="_abstract"]
To exchange an external OIDC token for a {productname} robot account token, you can first generate an OAuth2 token by using Red Hat Single Sign-On.
...
`<client_secret>`:: Specifies the Client Secret for the Client ID.
...
. Store the `access_token` from the previous step so that you can exchange it for a {productname} robot account token in the following procedure.
