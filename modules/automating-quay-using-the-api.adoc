...
Where:
+
* `import requests` includes the `requests` library in your Python code.
* `API_BASE_URL` is the URL of your registry appended with `/api/v1`.
* `ACCESS_TOKEN` is your OAuth 2 access token.
* `ORG_NAME` is the organization that holds the application.
* `"<admin_token_app>"` is the name of the application token to remain.
