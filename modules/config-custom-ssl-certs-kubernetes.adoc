...
[role="_abstract"]
To add custom SSL/TLS certificates to your {productname} deployment on Kubernetes, you can base64 encode the certificate, add it to the config secret, and restart the pods. 

The following procedure works around the limitation where the superuser panel certificate upload function does not work with Kubernetes deployments. 
