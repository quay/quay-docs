...
[role="_abstract"]
The following configuration fields are used for managing build logs in {productname}. These settings determine where build logs are archived, who can access them, and how they are stored.
...
| **LOG_ARCHIVE_PATH** | String | The path under the configured storage engine in which to place the archived build logs in `.JSON` format. **Example:** `archives/buildlogs`
|===
