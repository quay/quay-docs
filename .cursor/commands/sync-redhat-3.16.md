# Sync `redhat-3.16` with upstream
...
```bash
git fetch upstream
git checkout redhat-3.16
git rebase upstream/redhat-3.16
git push origin redhat-3.16 --force
git push upstream redhat-3.16 --force
git push downstream redhat-3.16 --force
```
