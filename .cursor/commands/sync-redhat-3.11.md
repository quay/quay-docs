# Sync `redhat-3.11` with upstream
...
```bash
git fetch upstream
git checkout redhat-3.11
git rebase upstream/redhat-3.11
git push origin redhat-3.11 --force
git push upstream redhat-3.11 --force
git push downstream redhat-3.11 --force
```
