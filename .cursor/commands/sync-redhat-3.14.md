# Sync `redhat-3.14` with upstream
...
```bash
git fetch upstream
git checkout redhat-3.14
git rebase upstream/redhat-3.14
git push origin redhat-3.14 --force
git push upstream redhat-3.14 --force
git push downstream redhat-3.14 --force
```
