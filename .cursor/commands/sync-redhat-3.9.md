...
2. Force-push the updated branch to `origin`, `upstream`, and `downstream` (same remotes as in the usual Quay docs workflow).

```bash
git fetch upstream
git checkout redhat-3.9
git rebase upstream/redhat-3.9
git push origin redhat-3.9 --force
git push upstream redhat-3.9 --force
git push downstream redhat-3.9 --force
```
