# Sync `redhat-3.10` with upstream
...
```bash
git fetch upstream
git checkout redhat-3.10
git rebase upstream/redhat-3.10
git push origin redhat-3.10 --force
git push upstream redhat-3.10 --force
git push downstream redhat-3.10 --force
```
