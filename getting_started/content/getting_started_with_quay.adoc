...
{productname} is an enterprise-quality container registry. Use Quay to build and store containers, then deploy them to the servers across your enterprise. Features of Quay include:
...
This getting started guide provides two different ways of setting up {productname} on Red Hat Enterprise Linux, both of which are based on Quay upstream documents:

* Set up {productname} (Basic) - Available in this guide, for non-production, test-only setups (based on link:https://coreos.com/quay-enterprise/docs/latest/initial-setup.html[For Testing as a container]).
* Set up {productname} (High Availability) - Soon to come, production-quality, high availability configuration (based on link:https://coreos.com/quay-enterprise/docs/latest/high-availability.html[High Availability for Quay Enterprise])
...
. **Identify the Redis hostname and add other desired settings**: Other setting you can add to complete the setup are as follows. For this basic, test configuration, identifying the Redis Hostname should be all you need to do.
+
...
. **Restart Quay**: When prompted, select "Restart Container" to restart Quay. Figure 4 shows that screen that appears as you want for Quay to restart.
...
Once the setup is complete, you can select Tutorial from the Quay home page to try the 15-minute tutorial. In the tutorial, you learn to log into Quay, start a container, create images, push repositories, view repositories, and change repository permissions with Quay.
