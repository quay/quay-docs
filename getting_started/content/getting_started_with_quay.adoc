...
* Set up Red Hat Quay (Basic) - Available in this guide, for non-production, test-only setups (based on link:https://coreos.com/quay-enterprise/docs/latest/initial-setup.html[For Testing as a container]).
* Set up Red Hat Quay (High Availability) - Soon to come, production-quality, high availability configuration (based on link:https://coreos.com/quay-enterprise/docs/latest/high-availability.html[High Availability for Quay Enterprise])
...
For a Red Hat Quay Registry installation (appropriate for non-production purposess), you need one system (physical or virtual machine) that has the following attributes:
...
1. **Install / Deploy link:https://access.redhat.com/containers/?tab=overview#/registry.access.redhat.com/rhscl/redis-32-rhel7)[Redis]**: Run Redis as a container:
...
3. **Identify the Redis hostname and add other desired settings**: Other setting you can add to complete the setup are as follows. For this basic, test configuration, identifying the Redis Hostname should be all you need to do.
+
